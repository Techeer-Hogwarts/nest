...
export class StudyTeamService {
    ...
    async ensureUserIsStudyMember(
        ...
        const isActive = await this.studyMemberService.isActiveStudyMember(
            studyTeamId,
            userId,
        );
        if (!isActive) {
            throw new StudyTeamNotActiveMemberException();
        }
    }
    ...
    async updateStudyTeam(
        ...
        const studyMembersToUpdate: StudyMemberInfoRequest[] = studyMember;
    ...
    private determineStudyMemberUpdates(
        ...
        this.logger.debug('delete:', deleteMembers);
        this.logger.debug(
            'toInactive: ',
